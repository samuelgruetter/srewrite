...
trait Rewriter {
  ...
  def show(what: Reflect#Tree, printerType: Rewriter.PrinterDescriptor = Rewriter.AFTER_TYPER, printMultiline: Boolean = false, decodeNames: Boolean = false) = {
    val buffer = new StringWriter()
    val writer = new PrintWriter(buffer)

    var printer = printerType match {
      case Rewriter.AFTER_NAMER => new PrettyPrinter(writer, printMultiline, decodeNames)
      case Rewriter.AFTER_TYPER | _ => new AfterTyperPrinter(writer, printMultiline, decodeNames)
    }
        ...
        case PackageDef(packaged, stats) =>
          contextManaged(tree){
            ...
            packaged match {
              ...
              case _ =>
                printAnnotations(tree)
                print("/*after-typer-package:*/ package ", packaged);
